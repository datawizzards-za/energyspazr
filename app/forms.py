...
class FinancierUpdateAccountForm(ModelForm):
    ...
    helper.layout = Layout(
        HTML('<h3 class="login-head"><i class="fa fa-lg fa-fw fa-user">' \
         '</i>FINANCIER ACCOUNT DETAILS</h3>'),
         Div(
             Div(
...
class PVTOrderForm(forms.Form):
    
    def __init__(self, appliance_choices, *args, **kwargs):
        super(PVTOrderForm, self).__init__(*args, **kwargs)
        self.fields['possible_appliances'].choices =  appliance_choices
    ...
    possible_appliances = forms.ChoiceField(choices=())
